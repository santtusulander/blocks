...
export const createGroup = createAction(GROUP_CREATED, (brand, account, name) => {
  return axios.post(`${urlBase}/AAA/brands/${brand}/accounts/${account}/groups`, {name: name}, {
    headers: {
...
export const deleteGroup = createAction(GROUP_DELETED, (brand, account, id) => {
  return axios.delete(`${urlBase}/AAA/brands/${brand}/accounts/${account}/groups/${id}`)
  .then(() => {
    return {id: id}
  });
})
...
export const fetchGroup = createAction(GROUP_FETCHED, (brand, account, id) => {
  return axios.get(`${urlBase}/AAA/brands/${brand}/accounts/${account}/groups/${id}`)
  .then((res) => {
    if(res) {
      return res.data;
    }
...
export const fetchGroups = createAction(GROUP_FETCHED_ALL, (brand, account) => {
  return axios.get(`${urlBase}/AAA/brands/${brand}/accounts/${account}/groups`)
  .then((res) => {
    if(res) {
      return res.data;
    }
...
export const updateGroup = createAction(GROUP_UPDATED, (brand, account, id, group) => {
  return axios.put(`${urlBase}/AAA/brands/${brand}/accounts/${account}/groups/${id}`, group, {
    headers: {
      'Content-Type': 'application/json'
    }
  })
