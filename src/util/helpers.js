...
export function buildAnalyticsOpts(params, filters, location ){
  ...
  const serviceProviderGroups =  visibleFilters.includes('service-provider') ? filters.get('serviceProviderGroups').size === 0 ? undefined : filters.get('serviceProviderGroups').toJS().join(',') : undefined

  const contentProviders = visibleFilters.includes('content-provider') ? filters.get('contentProviders').size === 0 ? undefined : filters.get('contentProviders').toJS().join(',') : undefined
  const contentProviderGroups = visibleFilters.includes('content-provider') ? filters.get('contentProviderGroups').size === 0 ? undefined : filters.get('contentProviderGroups').toJS().join(',') : undefined

  const serviceType = visibleFilters.includes('service-type') ? filters.get('serviceTypes').size > 1 ? undefined : filters.get('serviceTypes').toJS() : undefined
  const netType = visibleFilters.includes('service-type') ? filters.get('onOffNet').size > 1 ? undefined : filters.get('onOffNet').get(0).replace(/-.*$/, '') : undefined
  const errorCodes = visibleFilters.includes('error-code') ? filters.get('errorCodes').size === 0 || filters.get('errorCodes').size === httpErrorCodes.length ? undefined : filters.get('errorCodes').toJS().join(',') : undefined
  const statusCodes = visibleFilters.includes('error-code') ? filters.get('statusCodes').size === 0 || filters.get('statusCodes').size === httpStatusCodes.length ? undefined : filters.get('statusCodes').toJS().join(',') : undefined
