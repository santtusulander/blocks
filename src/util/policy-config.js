...
export function parsePolicy(rule, path) {
  ...
  // })
  return  {
    matches: parseConditions(rule.get('rule_body').get('conditions'), path.concat(['rule_body', 'conditions'])),
    sets: parseActions(rule.get('rule_body').get('actions'), path.concat(['rule_body', 'actions']))
  }
}
